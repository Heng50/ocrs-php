                ...
                <form action="../backend/course_controller.php" method="post" id="updateForm" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select id="prog_id"
                                ...
                                class="block w-full rounded-lg border border-gray-300 px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#FFD700] focus:border-[#FFD700] transition-colors duration-200"
                                required>
                            <option value="<?php echo htmlspecialchars($course['prog_id']); ?>" selected><?php echo htmlspecialchars($course['prog_name']); ?></option>
                            <?php
                                $sql = "SELECT * FROM programmes WHERE prog_id != ?";
                                $stmt = $pdo->prepare($sql);
                                $stmt->execute([$course['prog_id']]);
                                while($row = $stmt->fetch(PDO::FETCH_ASSOC)):
                            ?>
                            <option value="<?php echo htmlspecialchars($row['prog_id']); ?>"><?php echo htmlspecialchars($row['prog_name']); ?></option>
                            <?php endwhile; ?>
                        </select>
                    </div>
