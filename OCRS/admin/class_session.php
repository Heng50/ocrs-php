<?php
    ...
    // Get all class sessions with their basic info
    $sql = "SELECT class_sessions.*, c.course_code, c.course_name, u.username
    FROM class_sessions 
    JOIN courses c ON class_sessions.course_id = c.course_id 
    JOIN users u ON class_sessions.instructor_id = u.user_id
    ORDER BY class_sessions.session_id";
    $stmt = $pdo->query($sql);
?>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <?php while($class_session = $stmt->fetch(PDO::FETCH_ASSOC)): ?>
                                    <?php
                                    ...
                                    <tr class="hover:bg-gray-50 transition-colors duration-200">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900"><?php echo htmlspecialchars($class_session['username']); ?></td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900"><?php echo htmlspecialchars($class_session['semester']); ?></td>
